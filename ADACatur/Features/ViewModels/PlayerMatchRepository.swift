...
class PlayerMatchRepository {
    ...
    func fetchPlayerMatch(player: Player) async -> [PlayerMatch]  {
...
//                let matchDatabase = self.container.publicCloudDatabase
                ...
                do {
                    let matchRecords = try await database.perform(matchQuery, inZoneWith: nil)
...
//                let opponentDatabase = self.container.publicCloudDatabase
                ...
                do {
                    let opponentRecord = try await database.perform(opponentQuery, inZoneWith: nil)
...
//                        let playerOpponentDatabase = self.container.publicCloudDatabase
                        ...
                        do {
                            let playerOpponentRecord = try await database.perform(playerOpponentQuery, inZoneWith: nil)
...
//                                print("\(pRecord["name"]!)")
                ...
                allPlayerMatches.append(PlayerMatch(player: fetchedOpponent, match: fetchedMatch, result: returnResult, eloChange: record["eloChange"] as! Double))
            }
